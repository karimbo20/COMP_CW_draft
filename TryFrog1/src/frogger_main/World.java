...
/**
* A class definition for the game scene, it handles all object interactions with the world
* 
*
*/
public abstract class World extends Pane {
    ...
    /**
     * function to add an actor as a child to the front of the pane, which is the outer most layer
		...
		if (positionslots[i]=="") {
			...
			if (positionfull[i]>0) {
				if (positionslots[i].contentEquals(type)) {
					pos.x=positionxaxis[i]+(rand.nextInt(70)+150); //the x coordinate is dependent on how many items are already in the slot and on a random factor with just enough space to avoid overlap
					positionxaxis[i]=pos.x;
...
/**
 * this is where this script starts, and it calls to initialize and start the game timer
 */
public void start() {
	createTimer();
    timer.start();
